...
export default function Admin() {
  ...
  // CÁLCULO DAS ESTATÍSTICAS NO FRONTEND (Sincronia Total)
  const dashboardStats = useMemo(() => {
    if (!appointments || appointments.length === 0) {
      return { today: 0, total: 0, revenue: 0, pending: 0, popular: "Nenhum" };
    }

    const todayStr = new Date().toISOString().split('T')[0];
    const active = appointments.filter((a: any) => a.status !== 'cancelled');
    
    const todayCount = active.filter((a: any) => a.appointment_date === todayStr).length;
    const totalCount = active.length;
    
    const revenue = active
      .filter((a: any) => a.is_paid || a.isPaid)
      .reduce((sum: number, a: any) => sum + (Number(a.total_price || a.totalPrice) || 0), 0);
    ...
    active.forEach((a: any) => {
      const services = a.services_json || a.servicesJson || [];
    ...
    return { today: todayCount, total: totalCount, revenue, pending, popular };
  }, [appointments]);
  ...
  const markAsPaid = useCallback(async (appointmentId: number, methodOverride?: string) => {
    ...
    try {
      ...
      // Atualiza os dados da tabela (o cálculo do dashboard é automático via useMemo)
      refetchAppointments?.();
    } catch (err: any) {
    ...
    <div className="container mx-auto px-4 py-12">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-12">
        <Card className="bg-card border-border/50">
          ...
          <CardContent>
            {isLoadingAppointments ? <Skeleton className="h-8 w-16" /> : (
              <div className="text-3xl font-bold text-foreground">{dashboardStats.today}</div>
            )}
          </CardContent>
        </Card>
        ...
        <Card className="bg-card border-border/50">
          <CardHeader className="flex flex-row items-center justify-between pb-2">
            <CardTitle className="text-sm font-medium text-muted-foreground">
              Total Agendamentos ({monthNames[parseInt(selectedMonth) - 1]})
            </CardTitle>
        ...
        <Card className="bg-card border-border/50">
          <CardHeader className="flex flex-row items-center justify-between pb-2">
            <CardTitle className="text-sm font-medium text-muted-foreground">
              Receita ({monthNames[parseInt(selectedMonth) - 1]}/{selectedYear})
            </CardTitle>
            <DollarSign className="w-4 h-4 text-primary" />
          </CardHeader>
          <CardContent>
            {isLoadingAppointments ? <Skeleton className="h-8 w-24" /> : (
              <div className="flex flex-col gap-1">
                <div className="text-3xl font-bold text-primary leading-none">
                  {formatCurrencyFromCents(dashboardStats.revenue)}
                </div>
                <div className="text-xs font-semibold text-amber-500/80">
                  + {formatCurrencyFromCents(dashboardStats.pending)} a receber
                </div>
              </div>
            )}
          </CardContent>
        ...
        <Card className="bg-card border-border/50">
          <CardHeader className="flex flex-row items-center justify-between pb-2">
            <CardTitle className="text-sm font-medium text-muted-foreground">
              Serviço Popular ({monthNames[parseInt(selectedMonth) - 1]})
            </CardTitle>
            <Scissors className="w-4 h-4 text-primary" />
          </CardHeader>
          <CardContent>
            {isLoadingAppointments ? <Skeleton className="h-8 w-24" /> : (
              <div className="text-xl font-medium truncate text-foreground">{dashboardStats.popular}</div>
            )}
          </CardContent>
        </Card>
      </div>
